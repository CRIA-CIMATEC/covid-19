...
app.secret_key = 'secret_key_unique_and_personal'
cors = CORS(app)

ALLOWED_EXTENSIONS = set(['png', 'jpg', 'jpeg'])

paths = '/var/www/covid/models/'
...
def preditores_targets(np_treino, vp, vf):
    ...
    for i in identificador:
        ...
        if(vp + vf <= dados_treino_temp.shape[0]):
            ...
            print("Little dataset to create time series:" + str(i))
...
def pred_possibilidades_nf(regra, possibilidades, lb, lf, normalizador_x, normalizador_y, modelo_rna,complemento_df,lista_df_predicoes_cc,lista_df_predicoes_mc,lista_df_predicoes_cc_norm,lista_df_predicoes_mc_norm):
    ...
    for h in range(0,len(lista_df_cenarios)):
        ...
        #x_scaled_newshape = np.reshape(norm_dados_teste_1, (norm_dados_teste_1.shape[0],lb, len(x_columns)))
        ...
        #dRan1 = list(pd.date_range(start =str(data_inicio_pred), periods=30, freq ='D'))
        #df_predicao = pd.DataFrame(predicao.reshape(1,-1), columns=dRan1)
...
def xray_def(imgs_dict):
    ...
    for key, img in imgs_dict.items():
        image = Image.open((os.path.join('/var/www/covid/static/uploads/', img)))
        ...
        try:
            if (pred[0][0] > pred[0][1]) and (pred[0][0] > pred[0][2]):
                image = cv2.imread(os.path.join('/var/www/covid/static/uploads/', img))
...
def tc_def(imgs_dict):
    ...
    try:
        itens = {}
        for key,img in imgs_dict.items():
            imagens = dim_2d_preprocessing("/var/www/covid/static/uploads/"+img)
...
def dicom_def(imgs_dict):
    ...
    imagens = dim_3d_preprocessing(lista,"/var/www/covid/static/uploads/")
...
def simulation_cases_sird_teste():
    try:
        ...
        model = pysir.SIRD(int(populacao)) #bahia
...
def inflex_peak_api():
    ...
    colunas = ['id','name','confirmed','recovered','deaths','c11valor','c21valor','c31valor','c41valor','c51valor','c61valor','c71valor','c81valor']
...
def optimization_calculate():
    try:
        ...
        print('Started time: (hh:mm:ss.ms) {}'.format(start_time))

        #### PREDCÃO CASOS CONFIRMADOS
        ...
        for cenario in cenarios_possiveis:
            ...
            print("Cenário "+str(i), end=" ")
            i=i+1

        time_elapsed = datetime.datetime.now() - start_time 
        print('Confirmados: Time elapsed (hh:mm:ss.ms) {}'.format(time_elapsed))
        ...
        print('Time elapsed (hh:mm:ss.ms) {}'.format(time_elapsed_geral))
        ...
        return Response(response="Erro: {0}".format(str(ex)), status=400)
...
def xray_api():
    ...
    for image in images:     #image will be the key 
        ...
        images[image].save(os.path.join('/var/www/covid/static/uploads/', file_name))
    ...
    try:
        for image in images:
            os.remove('/var/www/covid/static/uploads/' + images[image].filename)
    except:
        pass
